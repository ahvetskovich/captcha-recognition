...
class KCAPTCHA{
	...
	function KCAPTCHA(){
		...
		$this->dirPath = "E:/GitHub/captcha-recognition/kcaptcha/captchas/";
		...
		do{
			...
			while(true){
				...
				$path = $this->dirPath.$this->keystring.".gif";
				if(!preg_match('/cp|cb|ck|c6|c9|rn|rm|mm|co|do|cl|db|qp|qb|dp|ww/', $this->keystring) && !file_exists($path))
					break;
			}
		...
		$resizedImg = imagecreatetruecolor($width/2, $height/2);
		imagecopyresized($resizedImg, $img2, 0, 0, 0, 0, $width/2, $height/2, $width, $height);
		
		imagegif($img2, "E:/GitHub/captcha-recognition/kcaptcha/original.gif");
		imagegif($resizedImg, "E:/GitHub/captcha-recognition/kcaptcha/resized.gif");
		$imgData = $this->keystring.",";
	
/* 		for($column=0;$column<$width;$column++)
			for($row=0;$row<$height;$row++){
				$rgb = imagecolorat($img2, $column, $row);
				$r = ($rgb >> 16) & 0xFF;
				$g = ($rgb >> 8) & 0xFF;
				$b = $rgb & 0xFF;
				$imgData .= $r.",".$g.",".$b.",";
...
/* 		header('Expires: Mon, 26 Jul 1997 05:00:00 GMT'); 
		...
		if(function_exists("imagegif")){
			...
			imagejpeg($img2, $path.".jpg", $jpeg_quality);
		} else if(function_exists("imagepng")){
			#header("Content-Type: image/x-png");
			imagepng($img2, $path.".png");
		}*/
		
/* 		$fp = fopen($outputFile, "a");
		$result = fwrite($fp, $imgData);
		fclose($fp);  */
	}
